...
if($_GET['status'] == 'new'){
    ...
    Db::query("UPDATE `users` SET `referal_hold`='".calcP($partner['referal_hold'], 20, $offer['leadPrice'], 'pay')."' WHERE `id`='".$partner['id']."'");
        ...
        // Начисление будет проходить по порядку 3-2-1
        User::updateBalance($user['id'], calcB($user['balance'], $offer['leadPrice'], 'pay'), calcB($user['hold'], $offer['leadPrice'], 'rej'));


        if(!empty($r1)) User::updateRefbalance($r1['id'], calcP($r1['referal_balance'], 20, $offer['leadPrice'], 'pay'), calcP($r1['referal_hold'], 20, $offer['leadPrice'], 'rej'));


        if(!empty($r2)) User::updateRefbalance($r2['id'], calcP($r2['referal_balance'], 20, $offer['leadPrice']), calcP($r2['referal_hold'], 20, $offer['leadPrice'], 'rej'));
